...
import { message, Progress, Select, Card, Button, List, Tag, Typography, Space, Divider } from 'antd';
import { DownloadOutlined, CheckOutlined, CloudDownloadOutlined } from '@ant-design/icons';
...
const SystemInfoPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        (window as any).electronAPI.onDownloadProgress(({ percent }: { percent: number }) => {
        ...
        (window as any).electronAPI.getFrpVersion().then(({ versions, currentVersion }: { versions: string, currentVersion: string | null }) => {
            if (versions) {
    ...
    const handleDownload = async () => {
        if (downloadVersion) {
            const downloadUrl = `https://github.com/fatedier/frp/releases/download/${downloadVersion}/frp_${downloadVersion.substring(1)}_${platform}_${arch}.tar.gz`;
            ...
            <Space direction="vertical" size="large" style={{ width: '100%' }}>
                <Card>
                    ...
                    <Space direction="vertical" style={{ width: '100%' }}>
                        <Text strong>操作系统: <Tag color="blue">{platform}</Tag></Text>
                        <Text strong>处理器架构: <Tag color="green">{arch}</Tag></Text>
                        <Text strong>
                            当前版本: 
                            {currentVersion ? (
                                <Tag color="processing" icon={<CheckOutlined />}>{currentVersion}</Tag>
                            ) : (
                ...
                <Card>
                    ...
                    <Space direction="vertical" style={{ width: '100%' }}>
                        <div>
                            ...
                            <div style={{ marginTop: 16, display: 'flex', gap: '10px', alignItems: 'center' }}>
                                <Select
                                    style={{ width: 200 }}
