...
// 解压缩函数
async function extractTarGz(filePath: string, destPath: string, onProgress: (progress: number) => void): Promise<void> {
...
function updateVersionFile(versionFile: string, newVersion: string) {
    ...
    versions.sort((a, b) => {
        ...
        if (versionA && versionB) {
            return parseInt(versionB[1]) - parseInt(versionA[1]) || parseInt(versionB[2]) - parseInt(versionA[2]) || parseInt(versionB[3]) - parseInt(versionA[3]);
        }
        return 0;
    });
